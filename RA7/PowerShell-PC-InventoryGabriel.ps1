<#
.SYNOPSIS
Mejorar el script para inventario de PCs en PowerShell
.DESCRIPTION
Mejorar el script para inventario de PCs en PowerShell con el objetivo de hacerlo más robusto y portable en un entorno de aula o red.
.PARAMETER [RutaComparteAula]
param(
    [switch]$c,

    # Carpeta de salida de resultados
    [string]$OutputPath = "H:\Practica_PS",

    # Carpeta donde se almacenarán los logs
    [string]$LogPath = "H:\Practica_PS\logs",

    # Código de sesión identificativo (por ejemplo: UT1_P1_GCA)
    [string]$SessionCode = "UT1_P1_GCA"
)
.EXAMPLE
No hay ejemplo
.NOTES
...
Write-Host "Obteniendo datos del sistema..." -ForegroundColor Cyan


Write-Verbose "Procesando información del equipo..."
Write-Host "Procesando información del equipo..." -ForegroundColor Cyan

# EJ 2

# Ruta alternativa: Documentos del alumno
$DocumentosPath = "$env:USERPROFILE\Documents\Practica_PS"
$DocumentosLogPath = "$DocumentosPath\logs"

# Variable para saber si usamos la ruta principal o la alternativa
$Ruta = $true

# 1º Comprobamos si la unidad H: existe
if (-not (Test-Path "H:\")) {
    Write-Warning "La unidad H: no está mapeada o no es accesible."
    $Ruta = $false
}
else {
    # 2º Si H: existe, comprobamos si podemos crear/usar la carpeta principal
    if (-not (Test-Path $OutputPath)) {
        # Intentamos crearla
        $CrearCarpeta = New-Item -Path $OutputPath -ItemType Directory -Force -ErrorAction SilentlyContinue
        if (-not $CrearCarpeta) {
            Write-Warning "No se pudo crear la carpeta en H:\Practica_PS"
            $Ruta = $false
        }
    }

    # 3º Si la carpeta principal existe o se creó, comprobamos la subcarpeta logs
    if ($Ruta) {
        if (-not (Test-Path $LogPath)) {
            $CrearCarpetaLogs = New-Item -Path $LogPath -ItemType Directory -Force -ErrorAction SilentlyContinue
            if (-not $CrearCarpetaLogs) {
                Write-Warning "No se pudo crear la subcarpeta logs en H:\Practica_PS"
                $Ruta = $false
            }
        }
    }
...
# =============================================
# Si algo falló → usamos Documentos como alternativa
# =============================================
if (-not $Ruta) {
    Write-Warning "Se cambiará a la carpeta Documentos como ruta alternativa."

    $OutputPath = $DocumentosPath
    $LogPath = $DocumentosLogPath

    # Creamos las carpetas en Documentos (aquí SÍ tenemos permisos siempre)
    if (-not (Test-Path $OutputPath)) {
    ...
    Write-Host "Ruta alternativa activada: $OutputPath" 
}
else {
    Write-Host "Todo correcto. Usando unidad H: → $OutputPath" 
}

# EJ 3

# Función que prueba si realmente se puede escribir en una carpeta
function Test-WriteAccess {
    param([string]$Folder)

    $Archivo = Join-Path $Folder ("test_" + (Get-Random) + ".tmp")

    # Intentamos crear el archivo
    Out-File -FilePath $Archivo -Encoding UTF8 -ErrorAction SilentlyContinue

    # Si se creó, intentamos borrarlo
    if (Test-Path $Archivo) {
        Remove-Item $Archivo -Force -ErrorAction SilentlyContinue
        if (-not (Test-Path $Archivo)) {
            return $true    # Sí se pudo escribir y borrar
        }
    }
    return $false           # No se pudo escribir o borrar
}
...
Write-Host ""
Write-Host "============================================"
Write-Host "   RESUMEN FINAL DEL INVENTARIO"
Write-Host "============================================"

if (Test-Path $csv) {
    Write-Host "✅ Inventario generado correctamente en: $csv" -ForegroundColor Green
} else {
    Write-Host "⚠️ Guardado local por falta de conexión." -ForegroundColor Yellow
}
...
function Write-Log {
    param (
        ...
        [Parameter(Mandatory)][string]$SessionCode,
        [string]$Equipo = $env:COMPUTERNAME
    )
    $ts = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
    $line = "[$ts] <$Nivel> <$Equipo> <$SessionCode> $Mensaje"
...
$csv = "D:\imagenes\Scripts\InventoryOutput\$env:computername-Inventory.csv"
...
$ErrorLogPath = "D:\imagenes\Scripts\PowerShell-PC-Inventory-Error-Log.log"
...
if (-not (Test-Path $csv))
{
  ...
  try {
    New-Item -ItemType "file" -Path $csv -Force
